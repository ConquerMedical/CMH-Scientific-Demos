...
const CFG={
  'abp':{name:'ABP · AI as a Biological Primitive',file:'ABP_session_report.html',anchor:'#abpFigure',results:['#modeResultTitle','#resultPanel','#contrastLine','#ground','#inverseSection'],labels:{rt:'Radiation',tz:'Agent intensity',wk:'Treatment week',abpEvidence:'Evidence substrate',inverseObjective:'Inverse objective'},displays:{rt:'vRt',tz:'vTz',wk:'vWk'}},
...
function placeDock(){
  ...
  function placeABP(){
    if(path!=='abp'){anchor.insertAdjacentElement('afterend',dock);return;}
    const portrait=window.matchMedia('(max-width:820px) and (orientation:portrait)').matches;
    const fig=document.getElementById('abpFigure'),panel=document.getElementById('resultPanel');
    if(portrait&&fig){fig.insertAdjacentElement('afterend',dock);}
    else if(panel){panel.appendChild(dock);}
  }
  placeABP();dock.appendChild(btn);
  if(path==='abp'){let rt=null;window.addEventListener('resize',()=>{clearTimeout(rt);rt=setTimeout(placeABP,100)},{passive:true});window.addEventListener('orientationchange',()=>setTimeout(placeABP,160),{passive:true});}
